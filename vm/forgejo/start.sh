...
TAP_NAME="vm_forge"
# TEAM_030: Bridge-based networking - all VMs on same 192.168.100.x subnet
...
# TEAM_025: Disable Phantom Process Killer (Android 12+)
...
pkill -9 -f "crosvm.*forge" 2>/dev/null || true
rm -f ${VM_DIR}/vm.sock
sleep 1

# TEAM_030: Setup shared bridge if not exists (SQL VM may have created it)
...
# Build kernel params
# TEAM_025: Use ttyS0 for console - crosvm --serial captures serial port, NOT virtio hvc0
